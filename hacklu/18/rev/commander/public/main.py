...
def load_kern(ctx):
  ...
  stack_low = 1
  stack_high = 0
  kern, elf = kernel.Kernel.LoadFrom(elf='./core.dump', arch=guess_arch('x86'))
...
class UCSolver:
  ...
  def __init__(self, kern):
    ...
    self.hook_addr(0x56557030, self.strlen_hook)
    self.hook_addr(0x56557267, self.cmp_hook)
    self.pwd = []

  def cmp_hook(self, *args):
    self.pwd.append(self.regs.cl)

  def strlen_hook(self, *args):
    content = self.mem.get_str(self.kern.mc.get_arg(1))
    print('GOT CONTENT ', content)
    self.kern.mc.ret_func(len(content)+50)
