...
int main() {
  ...
  if (1) {
    ...
    if (1) {
      szb = 0x1600;
      REP (f, 30) { tb[pos++] = malloc(szb); }
    } else {
      REP (i, 30) {
        REP (j, 2)
          tb[pos++] = malloc(((100 >> 6) + i) << 6);
      }
    }
    ...
    REP (i, pos) {
      void *c = tb[i];
      printf("%p %p xx diff=%Lx %Lx\n", b, c, (char *)b - (char *)c,
             ((u64 *)c)[2]);
    }

    return 0;
  }
  u64 **tb = (u64 **)malloc(0x1000);
  // u64 **tb1 = (u64 **)malloc(0x1000);
  u64 **tb2 = (u64 **)malloc(0x1000);
  u64 **tb3 = (u64 **)malloc(0x1000);
  // u64 **tb4 = (u64 **)malloc(3073 - 16);
  u64 ***tgt = (u64 ***)tb2;
  ...
  if (0) {
    ...
    REP (i, 100) {
      ...
      if (sz == 0 || choice) {
        int malloc_sz = 100;
        if (0) malloc_sz = rand() % 400 + 1;
        void *cur = malloc(malloc_sz);
        vals_ptr[nvals] = cur;
        vals[nvals++] = ((void **)cur)[1];
        alloc[sz++] = cur;
      } else {
        int u = rand() % sz;
        free(alloc[u]);
