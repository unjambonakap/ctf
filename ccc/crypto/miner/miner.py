    ...
    def update(self):
        if self.version >= self.max_version:
            ...
            for i in range(self.hash_length):
                self.s[i] = (self.s[i] * self.s[i]) % self.n
...
class SignatureScheme(object):
    ...
    def keygen(cls, bitlength, max_version, hash_function, hash_length):
        ...
        for i in range(hash_length):
            h = random_non_trivial_unit(n)
            u.append(inverse(pow(h, e, n), n))
            h = (h * h) % n
            s.append(h)
    ...
    def _hash(cls, hash_function, message, version, y):
        ...
        if sys.version_info[0] <= 2:  # Python 2 compatibility
            hash_value = [ord(c) for c in hash_value]
        return cls._binary(hash_value)
    ...
    def hash2(self, message, version):
        return self._hash(
            self.pk.hash_function,
            message,
            version,
    ...
    def _sign(self, message):
        ...
        result = self.sk.r
        for i in range(self.sk.hash_length):
            if hash[i] == 1:
                result = (result * self.sk.s[i]) % self.pk.n
    ...
    def _verify(cls, pk, hash_function, messages, signature, versions=None):
        ...
        for version in sorted(messages.keys(), reverse=True):
            squarings_to_do = pk.max_version + 1 - squarings_done - version
            exp = 2**(squarings_to_do)
            signature = pow(signature, exp, pk.n)
            hash = cls._hash(hash_function, messages[version], version, pk.y)
            if squarings_done == 0: print(hash)
            signature = (signature * pk.y) % pk.n
            for (u, b) in zip(pk.u, hash):
                if b == 1:
                    signature = (signature * u) % pk.n
            squarings_done += squarings_to_do
