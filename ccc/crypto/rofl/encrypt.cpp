...
struct RNG {
  ...
  uint64_t next_qword_safe() {
    uint64_t res = 0;
    for (int i = 0; i < 64; ++i) res |= next_bit() << i;
    return res;
  }

  // For when we don't care about security
  uint64_t next_qword_fast() { return rng(); }
};
...
struct Matrix {
  ...
  Matrix(int rows, int cols) : elements(rows, Vector(cols)) {}
  ...
  void fill(RNG *rng) {
    for (auto &row : elements)
      for (auto &x : row) x = rng->next_qword_fast();
  }
};
...
int main(int argc, const char **argv) {
  ...
  {
    RNG rng;
    for (auto &x : key) x = rng.next_qword_safe();
  }
  ...
  for (auto x : cipher) write64(out, x);
